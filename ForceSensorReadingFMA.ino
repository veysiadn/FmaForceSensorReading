/*
 *  Wed Aug 25 14:29:10 2021
 *  Copyright (C) 2021 Veysi ADIN, UST KIST
 *  Contact information: veysi.adin@kist.re.kr
 *  This ino file is written by Veysi ADIN for reading force data from Honeywell FMAMSDXX015WCSC3CT-ND  force sensor via SPI.
*/

#include <SPI.h>
// Sensor select pin for activation
// Force sensor part number : FMAMSDXX015WCSC3CT-ND 
// 15 N, SPI, 3V3 Supply, %20 to %80 Transfer function; 
...
// ------------------ First sensor reading started ------------------------------------------
  ...
  if(!sensor1_status){
    ...
    force_val = ((sensor1_force_data-output_min)*15.0)/(output_max-output_min);
...
//
  ...
  if(!sensor2_status){
    ...
    // Conversion of 14 bit force readout to Newton.
    force_val2 = ((sensor2_force_data-output_min)*15.0)/(output_max-output_min);
    Serial.print(" Sensor 2 force value= ");
    Serial.println(force_val2);
    //Serial.print("  | Temperature value= ");
